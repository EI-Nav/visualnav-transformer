"""ViNT model inference wrapper for NavArena (ImageNav)."""
...
class ViNTInference:
    """ViNT forward pass with temporal context queues per slot."""
    ...
    def __init__(
        ...
        if self._config.get("model_type") != "vint":
            raise ValueError(
                f"vint_server expects model_type 'vint', got {self._config.get('model_type')!r}"
            )
        ...
        logger.info(
            "ViNTInference: dataset=%s metric_scale=%.4f normalize=%s context_size=%d "
    ...
    def _obs_goal_tensors(
        ...
        """Stack context + goal the same way as training (``transform_images`` per frame then concat)."""
