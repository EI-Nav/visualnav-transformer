...
class ViNT_Dataset(Dataset):
    ...
    def _build_index(self, use_tqdm: bool = False):
        ...
        for traj_name in tqdm.tqdm(self.traj_names, disable=not use_tqdm, dynamic_ncols=True):
            ...
            for curr_time in range(begin_time, end_time):
                max_goal_distance = min(self.max_dist_cat * self.waypoint_spacing, traj_len - curr_time - 1) # 当前时刻可采样的最大目标距离
    ...
    def _compute_actions(self, traj_data, curr_time, goal_time):
        ...
        # Ensure proper numeric types from the start
        ...
        if yaw.shape != (self.len_traj_pred + 1,):
            ...
            # Ensure proper dtype when repeating
            yaw = np.concatenate([yaw, np.full(const_len, yaw[-1], dtype=yaw.dtype)])
        ...
        # Ensure actions is a proper numeric array, not object type
