"""CLI entry point: ``python -m gnm_server``."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="GNM NavArena evaluation server",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument(
        "--config",
        default=str(REPO_ROOT / "train" / "config" / "gnm.yaml"),
        help="Path to GNM training YAML",
    )
    p.add_argument(
        "--checkpoint",
        default=str(REPO_ROOT / "deployment" / "model_weights" / "gnm.pth"),
        help="Path to GNM checkpoint (.pth); default matches deployment/config/models.yaml",
    )
    p.add_argument(
        "--data-config",
        default=str(
    ...
    p.add_argument("--port", type=int, default=8767, help="Server port")
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Loading GNM model from %s", args.checkpoint)
    inference = GNMInference(
    ...
    server = GNMServer(inference=inference, controller=controller)

    logger.info("Starting GNM server on ws://%s:%d", args.host, args.port)
